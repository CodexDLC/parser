...
1. Celery Beat запускает `parse_enabled_sources`.
2. Задача получает все включенные источники.
...
- Не вызывать внешние API напрямую из эндпоинтов, кроме ручной тестовой генерации.
